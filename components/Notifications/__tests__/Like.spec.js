...
import NotificationLike from '../Includes/Like.vue';
import { notifyLike } from '../../../test/notifications.mock';

Wrapper.prototype.ref = function(id) {
	return this.find({ ref: id });
};

describe('Notification Like', () => {
	let wrapper;
	...
	beforeEach(() => {
		...
		wrapper = shallowMount(NotificationLike, {
			mocks: {
				$t: msg => msg,
				_i18n: {
					locale: 'en',
	...
	it('should go to post landing page by clicking on post title', () => {
		...
		expect($router.push).toHaveBeenCalledWith(`/post/${notifyLike.postId}`);
	});
});
